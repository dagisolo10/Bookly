...
export default function BusinessCard({ business, linkPath, index }: { business: FullBusiness; linkPath: string; index: number }) {
    return (
        <Link href={`${linkPath}/${business.id}`} className="group">
            <Card className="h-full gap-0 border-none bg-transparent p-2 shadow-none transition-all">
                ...
                <CardContent className="flex items-center justify-between px-1 py-2 text-left">
                    ...
                    {business.status !== "Active" && (
                        <div
                            className={cn(
                                business.status === "Paused" ? "bg-blue-100 text-blue-500" : "bg-red-100 text-red-500",
